...
// C++ std::map is not threadsafe for writing
  ...
  // LAIK initialization for SpMV (if instance provided)
  if (A.inst && A.world) {
    ...
    A.x_blob = init_blob(A, true, (char*)"x", A.local, A.ext);
    A.b_blob = init_blob(A, false, (char*)"b", A.local, A.ext);

    // copy local x/b values into LAIK buffers
    if (x != 0) {
      double* xv = 0; uint64_t xcount = 0;
      laik_get_map_1d(A.x_blob->values, 0, (void**)&xv, &xcount);
      for (local_int_t i = 0; i < A.localNumberOfRows; ++i) xv[i] = x->values[i];
    }
    if (b != 0) {
      double* bv = 0; uint64_t bcount = 0;
      laik_get_map_1d(A.b_blob->values, 0, (void**)&bv, &bcount);
      for (local_int_t i = 0; i < A.localNumberOfRows; ++i) bv[i] = b->values[i];
    }

    // Build CSR in LAIK with variable layout
    Laik_Space* rows_space = laik_new_space_1d(A.inst, A.totalNumberOfRows);
    Laik_Space* prefix_space = laik_new_space_1d(A.inst, A.totalNumberOfRows + 1);

    A.rowD = laik_new_data(prefix_space, laik_Int64);
    A.valD = laik_new_data(rows_space, laik_Double);
    A.colD = laik_new_data(rows_space, laik_Int64);

    Laik_Data_Parameters* vparams_val = (Laik_Data_Parameters*)malloc(sizeof(*vparams_val));
    vparams_val->prefix_row_data = A.rowD;
    vparams_val->vector_local_indices = 0;
    vparams_val->vector_local_count = 0;
    vparams_val->vector_external_indices = 0;
    vparams_val->vector_external_count = 0;

    Laik_Data_Parameters* vparams_col = (Laik_Data_Parameters*)malloc(sizeof(*vparams_col));
    vparams_col->prefix_row_data = A.rowD;
    vparams_col->vector_local_indices = 0;
    vparams_col->vector_local_count = 0;
    vparams_col->vector_external_indices = 0;
    vparams_col->vector_external_count = 0;
    ...
    // Initialize rowD/valD/colD on master, then distribute (like examples/spmv.c)
    Laik_Partitioner* master_pr = laik_new_master_partitioner();
    Laik_Partitioning* row_master = laik_new_partitioning(master_pr, A.world, prefix_space, 0);
    Laik_Partitioning* rows_master = laik_new_partitioning(master_pr, A.world, rows_space, 0);

    if (laik_size(A.world) == 1) {
      A.rowP = row_master;
      A.rowsP = rows_master;
    } else {
      Laik_Partitioner* rows_pr = laik_new_partitioner("rows_pr", partitioner_alg_for_rows, (void*)local_pd, LAIK_PF_None);
      A.rowP  = laik_new_partitioning(rows_pr, A.world, prefix_space, NULL);
      A.rowsP = laik_new_partitioning(rows_pr, A.world, rows_space, NULL);
    }
    ...
    // Now that rowD is initialized, allocate/fill valD and colD
    ...
    // Distribute to row/rows partitionings
    if (laik_size(A.world) > 1) {

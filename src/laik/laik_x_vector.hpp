...
struct partition_data
{
    ...
    const global_int_t *externalLocalToGlobal;         /* external local->global map (size numberOfExternalValues) */
    local_int_t numberOfExternalValues;                /* number of external values */
...
extern Laik_Blob *init_blob(const SparseMatrix &A, bool exchangesValues, const char * name);
extern Laik_Blob *init_blob(const SparseMatrix &A, bool exchangesValues, const char * name,
                            Laik_Partitioning *localP, Laik_Partitioning *extP);
/*
